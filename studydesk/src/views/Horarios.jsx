...
import { inputStyle, btnPrimary } from "../styles/shared";
...
export default function Horarios({ materias, onAddHorario, onDelHorario }) {
  ...
  const handleAdd = (materiaId) => {
    onAddHorario(materiaId, form);
  };

  return (
    <div>
      <div style={{ marginBottom: 26 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, margin: 0 }}>Horarios de materias</h1>
        <p style={{ color: dark.muted, fontSize: 14, marginTop: 4 }}>
          Administrá los bloques horarios de cada materia.
        </p>
      </div>
        ...
        return (
          <div key={m.id} style={{ background: dark.card, borderRadius: 16, border: `1.5px solid ${dark.border}`, padding: "20px 22px", marginBottom: 18 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 16 }}>
              {(m.horarios || []).map((h, i) => (
                <div key={i} style={{ background: COLOR_BG[ci], border: `1px solid ${COLORS[ci]}44`, borderRadius: 10, padding: "8px 14px", display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <button onClick={() => onDelHorario(m.id, i)} style={{ background: "transparent", border: "none", color: "#f87171", cursor: "pointer", fontSize: 14 }}>✕</button>
                </div>
              ))}
            </div>
            ...
            <div style={{ background: dark.subtle, borderRadius: 10, padding: "14px 16px" }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: dark.muted, marginBottom: 10, textTransform: "uppercase", letterSpacing: 0.8 }}>
                Agregar bloque
              </div>
              <div style={{ display: "flex", gap: 10, alignItems: "flex-end", flexWrap: "wrap" }}>
                ...
                <button onClick={() => handleAdd(m.id)} style={{ ...btnPrimary, padding: "10px 18px" }}>
                  + Agregar
                </button>
              </div>
            </div>
