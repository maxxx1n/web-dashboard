...
export default function Calendario({ tareas, recordatorios, getMColor, getMBg, getMText, getMNombre, onNewRecordatorio, onEditRecordatorio, onDelRecordatorio }) {
  ...
  const navBtn = { background: dark.card, border: `1.5px solid ${dark.border}`, color: dark.text, borderRadius: 8, width: 34, height: 34, cursor: "pointer", fontSize: 16 };
        ...
        <div style={{ background: dark.card, borderRadius: 16, border: `1.5px solid ${dark.border}`, overflow: "hidden" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)" }}>
            {dias.map((d, i) => {
              ...
              return (
                <div key={i} onClick={() => d && setCalDia(calDia === d ? null : d)}
                  style={{ minHeight: 82, padding: "8px 6px", borderRight: (i+1)%7!==0 ? `1px solid ${dark.border}` : "none", borderBottom: `1px solid ${dark.border}`, background: sel ? "#2d2b4e" : tod ? "#1e1e30" : "transparent", cursor: d ? "pointer" : "default", transition: "background 0.12s" }}>
                  {d && <>
                    ...
                    {tt.slice(0,1).map(t => (
                      <div key={t.id} style={{ fontSize: 10, background: getMColor(t.materiaId), color: "#0f0f13", borderRadius: 4, padding: "2px 5px", marginBottom: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontWeight: 600 }}>{t.titulo}</div>
                    ))}
                    {rr.slice(0,1).map(r => (
        ...
        {calDia && (
          <div style={{ background: dark.card, borderRadius: 16, border: `1.5px solid ${dark.border}`, padding: 20, display: "flex", flexDirection: "column" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
              ...
              <button onClick={() => setCalDia(null)} style={iconBtn(dark.muted)}>✕</button>
            </div>

            <button onClick={() => onNewRecordatorio({ fecha: diaSelStr, hora: "09:00" })}
              style={{ ...btnPrimary, fontSize: 12, padding: "8px 14px", marginBottom: 16, width: "100%" }}>
              + Recordatorio para este día
            </button>
            ...
            {recDia.length > 0 && <>
              ...
              {recDia.map(r => (
                <div key={r.id} style={{ background: "#3d2e0a", borderRadius: 10, padding: "10px 12px", marginBottom: 8, display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 600, color: "#fbbf24" }}>🔔 {r.titulo}</div>
                  ...
                  <div style={{ display: "flex", gap: 4 }}>
                    <button onClick={() => onEditRecordatorio(r)} style={iconBtn("#fbbf24")}>✎</button>
                    <button onClick={() => onDelRecordatorio(r.id)} style={iconBtn("#f87171")}>✕</button>
                  </div>
                </div>
              ))}
            </>}
