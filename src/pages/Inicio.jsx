...
export default function Inicio({
  ...
  const hoy = tareas.filter(
    (t) => t.fecha === todayStr && t.estado !== "hecha",
  );
      ...
      <div className="page-header">
        <div>
          ...
          <p className="welcome-summary">
            Tenés{" "}
            <strong style={{ color: "var(--accent)" }}>{pendientes}</strong>{" "}
            tarea{pendientes !== 1 ? "s" : ""} pendiente
            {pendientes !== 1 ? "s" : ""}.
            {hoy.length > 0 && (
              <>
                {" "}
          ...
          {tareas.slice(0, 6).map((t) => (
            <div key={t.id} className="task-inline">
              <div
                ...
                onClick={() =>
                  setEstado(t.id, t.estado === "hecha" ? "pendiente" : "hecha")
                }
              >
                {t.estado === "hecha" && (
                  <span
                    style={{ color: "#0c0c10", fontSize: 11, fontWeight: 800 }}
                  >
                    ✓
                  </span>
                )}
