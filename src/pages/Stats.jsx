...
export default function Stats({ tareas, materias, getMColor }) {
  ...
  const byMateria = materias
    .map((m) => ({
      ...
      hechas: tareas.filter((t) => t.materiaId === m.id && t.estado === "hecha")
        .length,
        ...
        {[
          ...
          <div key={idx} className="card card-hover metric-card">
            ...
            <div
              ...
              style={{ color: item.color, fontSize: 38 }}
            >
              {item.value}
            </div>
          </div>
              ...
              <div key={prioridad} style={{ marginBottom: 18 }}>
                ...
                >
                  <span
                    className="badge"
                    style={{
                      background: PRIORITY[prioridad].bg,
                      color: PRIORITY[prioridad].color,
                    }}
                  >
      ...
      <div className="card">
        ...
        {byMateria.length === 0 && (
          <div style={{ color: "var(--text-muted)", fontSize: 13 }}>
            No hay tareas asociadas a materias todavía.
          </div>
        )}
        {byMateria.map((m) => {
          const completedPct = m.total
            ? Math.round((m.hechas / m.total) * 100)
            : 0;
          return (
