...
import { Bell, Pencil, Trash2, Plus, X, Download } from "lucide-react";
...
export default function Calendario({
  ...
  const handleExportICS = () => {
    let icsContent =
      "BEGIN:VCALENDAR\nVERSION:2.0\nPRODID:-//Calendario App//EN\n";

    tareas.forEach((t) => {
      if (!t.fecha) return;
      const start = t.fecha.replace(/-/g, "");
      icsContent += "BEGIN:VEVENT\n";
      icsContent += `UID:tarea-${t.id}@calendario\n`;
      icsContent += `DTSTAMP:${start}T000000Z\n`;
      icsContent += `DTSTART;VALUE=DATE:${start}\n`;
      icsContent += `SUMMARY:${t.titulo}\n`;
      icsContent += `DESCRIPTION:Materia: ${getMNombre(t.materiaId)}\n`;
      icsContent += "END:VEVENT\n";
    });

    recordatorios.forEach((r) => {
      if (!r.fecha) return;
      const start = r.fecha.replace(/-/g, "");
      icsContent += "BEGIN:VEVENT\n";
      icsContent += `UID:rec-${r.id}@calendario\n`;
      icsContent += `DTSTAMP:${start}T000000Z\n`;
      if (r.hora) {
        const time = r.hora.replace(":", "") + "00";
        icsContent += `DTSTART;TZID=America/Argentina/Buenos_Aires:${start}T${time}\n`;
      } else {
        icsContent += `DTSTART;VALUE=DATE:${start}\n`;
      }
      icsContent += `SUMMARY:🔔 ${r.titulo}\n`;
      if (r.nota) icsContent += `DESCRIPTION:${r.nota}\n`;
      icsContent += "END:VEVENT\n";
    });
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <button
            ...
            onClick={() =>
              onNewRecordatorio({ fecha: todayStr, hora: "09:00" })
            }
          >
            <Bell size={14} /> Nuevo Recordatorio
          </button>
          <button className="nav-btn" onClick={prevMes}>
            ‹
          </button>
          <span
            style={{
          ...
          <button className="nav-btn" onClick={nextMes}>
            ›
          </button>
        </div>
      </div>
      ...
      <div
        style={{
          display: "grid",
          gridTemplateColumns: calDia ? "1fr 300px" : "1fr",
          gap: 18,
        }}
      >
        {/* Calendar grid */}
          ...
          <div className="cal-grid">
            {dias.map((d, i) => {
              ...
              return (
                ...
                >
                  <div
                    className={`cal-day-num${tod ? " today" : ""}`}
                    style={{
                      color: sel && !tod ? "var(--accent-soft)" : undefined,
                    }}
                  >
                    {d}
                  ...
                  {tt.slice(0, 1).map((t) => (
                    <div
                      ...
                      style={{
                        background: getMColor(t.materiaId),
                        color: "#0c0c10",
                      }}
                    >
                      {t.titulo}
                  ...
                  {rr.slice(0, 1).map((r) => (
                    <div
                      key={r.id}
                      className="cal-event-pill"
                      style={{ background: "#3d2e0a", color: "#fbbf24" }}
                    >
                      🔔 {r.titulo}
                    </div>
                  ))}
                  {tt.length + rr.length > 2 && (
                    <div style={{ fontSize: 10, color: "var(--text-muted)" }}>
                      +{tt.length + rr.length - 2} más
                    </div>
                  )}
                </div>
              );
        ...
        {calDia && (
          <div className="cal-sidebar">
            <div className="cal-sidebar-header" style={{ marginBottom: 16 }}>
              <div>
                <div className="cal-sidebar-title">
                  {calDia} de {MESES[calMes]}
                </div>
                <div className="cal-sidebar-count">
                  {tareasDia.length + recDia.length} eventos
                </div>
              </div>
              <button
                className="icon-btn"
                style={{ color: "var(--text-muted)" }}
                onClick={() => setCalDia(null)}
                title="Cerrar"
              >
                <X size={16} />
              </button>
            ...
            <button
              ...
              onClick={() =>
                onNewRecordatorio({ fecha: diaSelStr, hora: "09:00" })
              }
            >
            ...
            {recDia.length > 0 && (
              <>
                <div className="cal-section-label" style={{ color: "#fbbf24" }}>
                  Recordatorios
                </div>
                {recDia.map((r) => (
                  <div
                  ...
                  >
                    ...
                    <div style={{ display: "flex", gap: 4 }}>
                      ...
                      <button
                        ...
                        style={{ color: "#f87171" }}
                        onClick={() => onDelRecordatorio(r.id)}
                        title="Eliminar"
                      >
                        <Trash2 size={13} />
                      </button>
            ...
            {tareasDia.length > 0 && (
              <>
                <div
                  className="cal-section-label"
                  style={{
                    color: "var(--text-muted)",
                    marginTop: recDia.length ? 8 : 0,
                  }}
                >
                  Tareas
                </div>
                  ...
                  <div key={t.id} className="cal-task">
                    ...
                    <div style={{ display: "flex", gap: 6, marginTop: 4 }}>
                      <span
                        className="badge"
                        style={{
                          background: getMBg(t.materiaId),
                          color: getMText(t.materiaId),
                        }}
                      >
                        {getMNombre(t.materiaId)}
                      </span>
                    </div>
                  </div>
                ))}
